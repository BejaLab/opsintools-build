...
class TrimmedChain(PDB.Select):
    # from https://github.com/google-deepmind/alphafold3/blob/main/src/alphafold3/constants/residue_names.py
    ...
    def accept_residue(self, residue):
        res_is_aa = residue.resname in TrimmedChain.protein_letters_3to1
        if self.start <= residue.id[1] <= self.end and res_is_aa:
            if self.prev_res_is_aa == False:
                raise ValueError(f"Error: a residue has been skipped before resi {residue.id[1]}")
            self.residues.append(residue)
            return True
        self.prev_res_is_aa = res_is_aa
    def get_seq(self):
        seq = [ TrimmedChain.protein_letters_3to1[res.resname] for res in self.residues ]
        return ''.join(seq)
